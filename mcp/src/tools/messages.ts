...
export function registerMessageTools(server: McpServer, client: McpClient): void {
  ...
  server.registerTool(
    "get_message",
    {
      ...
      annotations: { readOnlyHint: true },
      description:
        "Use after `list_messages` to read one message in full — inbound or outbound (for outbound, this is also how you poll a send's terminal outcome) — body (text + html), header_from, envelope_from, verified_domain, SPF/DKIM/DMARC authentication evidence, conversation id, and attachment metadata. A non-null verified_domain means DMARC passed for that RFC 5322 From domain; it does not authenticate the mailbox local part, a person, or message content. Pass the message's `id` from the list response. **Side effect:** fetching an unread inbound message marks it read — there is no peek-without-consuming and no mark-unread, so only open a message when you mean to consume it. Attachment bytes are NOT included (would blow context for any non-trivial PDF); the response lists each attachment's filename, content_type, and 0-based `index` plus size_bytes. To get the actual bytes of one attachment (inspect, forward, hand off), call `get_attachment` with that index. The raw MIME blob is also omitted for the same reason.",
      inputSchema: strictInputSchema({
        message_id: z.string(),
        email: z.string().optional(),
      }),
