...
|---|---|---|---|
| `email.received` | `EmailReceivedData` | `message_id`, `agent_email`, `direction` (`inbound`), `header_from` (nullable RFC 5322 From), `envelope_from` (nullable SMTP MAIL FROM), `authentication` (nullable for providerless delivery; trust only DMARC `pass`), `to[]`, `cc[]`, `reply_to[]`, `delivered_to` (scalar — the one per-agent copy; the fetch key), `subject`, `received_at` | `conversation_id`, `attachments[]` (metadata only: `filename`, `content_type`, `size_bytes` — the DECODED payload size, `index`) |
