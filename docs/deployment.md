...
| `E2A_SMTP_PROXY_TRUSTED_CIDRS` | no (default empty) | Comma-separated CIDRs allowed to present PROXY protocol headers on the SMTP listener (e.g. `172.30.0.0/24,10.0.0.0/8` for HAProxy in front); empty disables parsing. Mirrors `smtp.proxy_trusted_cidrs`. |
