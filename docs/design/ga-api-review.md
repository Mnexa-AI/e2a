...
- ⏳ Remaining: **Slice C** (path `{address}`→`{email}`), **Slice D** (real pagination: suppressions + conversations), **Slice E** (`make generate-sdk` + SDK hand-layer = the deferred Phase-2 review).
