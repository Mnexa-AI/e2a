...
- **`GET /v1/account/export`** — returns a JSON dump of everything the authenticated account owns. Profile, agents, domains, API key metadata, all messages with bodies, usage events, and account-wide or exact-agent suppressions. Export schema v4 retains the v3 suppression-entry contract: optional `agent_email` distinguishes exact-agent blocks, while entries without it are account-wide. Internal identifiers (Google subject, key hashes, session tokens) are excluded.
