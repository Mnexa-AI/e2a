...
| Per-webhook signing secret (`whsec_…`) | Postgres, **plaintext** | Until the webhook is deleted. Returned once at creation; rotate via `POST /v1/webhooks/{webhook_id}/rotate-secret` (the previous secret stays valid for a 24h grace window). |
