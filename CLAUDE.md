...
- **Go test tiers**: `test-unit` needs no DB (headers, outbound, relay, config, webhook, approvaltoken, limits, httpapi, ratelimit). `test-integration` needs Postgres (identity, agent, hitlworker, hitlnotify, limits, relay). `test-e2e` discovers every package containing an `integration`-tagged test and runs it with that tag. `make test` runs everything (including e2e) with `-tags integration -p 1`. `make cover-check` enforces per-package coverage floors from `.testcoverage.yml`.
