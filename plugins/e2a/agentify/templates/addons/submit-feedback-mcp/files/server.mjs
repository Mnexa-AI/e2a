...
import { z } from 'zod';
import { validateFeedback, composeFeedbackEmail, statusFromThread, KINDS } from './bridge.mjs';
...
// Per-process, per-window rate cap — a backstop, not the durable limiter.
const hits = [];
function rateOk() {
  const now = Date.now();
  while (hits.length && now - hits[0] > 3_600_000) hits.shift();
  if (hits.length >= RATE) return false;
  hits.push(now);
  return true;
}
...
server.registerTool(
  ...
  async ({ kind, title, body }) => {
    ...
    if (!rateOk()) return { content: [{ type: 'text', text: 'RATE_LIMITED: too many feedback submissions this hour' }], isError: true };
    ...
    const msg = await e2a('POST', `/v1/agents/${FROM}/messages`, { to: [SUPPORT], subject, body: text });
