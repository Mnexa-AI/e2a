...
func TestBuildInboundReleasedEvent_Envelope(t *testing.T) {
	...
	ev := a.buildInboundReleasedEvent(msg, "u_owner")
	...
	// UserID is the routing key (the owner's webhooks must fire).
	if ev.UserID != "u_owner" {
		t.Errorf("UserID = %q, want u_owner", ev.UserID)
	}
	...
	if data["reviewed_by_user_id"] != "u_owner" {
		t.Errorf("reviewed_by_user_id = %v, want u_owner", data["reviewed_by_user_id"])
	}
}
...
func TestBuildInboundRejectedEvent_Envelope(t *testing.T) {
	...
	ev := a.buildInboundRejectedEvent(msg, "u_owner", "prompt injection")
