...
// SharedDomain is the fixed shared domain for slug-based agent registration.
const SharedDomain = "agents.e2a.dev"
...
func NewAPI(store *identity.Store, sender *outbound.Sender, smtpRelay *outbound.SMTPRelay, userAuth *auth.UserAuth, usage usage.UsageTracker, smtpDomain, fromDomain string, production bool) *API {
	return &API{
		store:         store,
		sender:        sender,
		smtpRelay:     smtpRelay,
...
// @Description  Register a new agent with a custom domain or a slug on agents.e2a.dev. Use `slug` for instant onboarding (no DNS needed), or `email` for a custom domain (requires domain to be registered and verified first).
// @Tags         Agents
// @Accept       json
// @Produce      json
// @Security     BearerAuth
...
func (a *API) handleRegisterAgent(w http.ResponseWriter, r *http.Request) {
	...
	if req.Slug != "" {
		...
		req.Email = req.Slug + "@" + SharedDomain
		isSharedDomain = true
	}
	...
	// For shared-domain agents, the domain is the shared domain (e.g. "agents.e2a.dev").
...
// @Produce      json
// @Security     BearerAuth
// @Param        email path string true "Agent email address" example(my-bot@agents.e2a.dev)
