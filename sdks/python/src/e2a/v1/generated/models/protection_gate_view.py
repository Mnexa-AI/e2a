...
class ProtectionGateView(BaseModel):
    ...
    policy: Optional[StrictStr] = Field(default='open', description="Trust gate: open (all), domain (inbound: senders on the listed domains; outbound: recipients on the agent's own domain), allowlist (listed addresses). Open set: new values may be added over time, so treat these as strings and tolerate unknown values. Known values: open, allowlist, domain.")
